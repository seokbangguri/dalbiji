...
$(function(){
		...
		if(data.length > 0){
			...
			$(".contact-list").empty();
			$(".contact-list").append("<li><span style=\"width: 100%; text-align: center;\" class=\"contact-name text-ellipsis\">"+"일정을 등록해주세요."+"</span></li>");
		}
...
window.onload = function() {
	  ...
	  var targetButton = $(".date-btn:has(#date:contains("+ parseInt(main.date) +"))");
	  var offset = targetButton.offset().left;

	  $(".date_content").animate({
		  scrollLeft: offset		  
	  }, 500);
	  console.log('스크롤 후');
}
